...
const UncleAppInternal = () => {
  ...
  useEffect(() => {
    if (isTxSuccess && currentTxHash && pendingTransaction) {
      setTransactions(prev => prev.map(tx => (tx.hash === currentTxHash ? { ...tx, status: "success" as const } : tx)));
  ...
  useEffect(() => {
    if (isTxError && currentTxHash) {
      setTransactions(prev => prev.map(tx => (tx.hash === currentTxHash ? { ...tx, status: "failed" as const } : tx)));
  ...
  const handleVouchAction = async (vouchedForBorrowerName: string, vouchedAmount: number) => {
    ...
    try {
      ...
      setUserLoanDetails(prev => ({
        ...
        loanStatus:
          prev.amountActuallyFundedByVouchers + vouchedAmount >= prev.originalLoanAmount ? "active" : prev.loanStatus,
      }));
  ...
  const NetworkStatus = () => {
    ...
    return (
      <div
        className={`fixed top-4 left-4 px-3 py-2 rounded-lg text-sm font-medium z-50 ${
          isCorrectNetwork ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
        }`}
      >
        {isCorrectNetwork ? `✓ Flow Testnet (${chain.name})` : `⚠ Wrong Network: ${chain.name}`}
      </div>
    );
  };
  ...
  if (!isConnected) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-sm text-gray-800 mb-4">Use the RainbowKit button in the top right corner</p>
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md">
          <p className="text-sm text-blue-800">
            <strong>Make sure to connect to Flow EVM Testnet!</strong>
            <br />
            Chain ID: 545
            <br />
        ...
        {pendingTransaction && (
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <p className="text-xs text-blue-600 mt-2">This may take a few seconds to confirm...</p>
            </div>

            {currentTxHash && (
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <p className="text-sm text-gray-800 mb-2">Transaction Hash:</p>
                <div className="flex items-center space-x-2">
                  <code className="text-xs bg-white px-2 py-1 rounded border flex-1 truncate">{currentTxHash}</code>
                  <a
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto">
        ...
        <button
          ...
          className="mt-8 text-gray-800 text-sm"
        >
          Switch Mode
        </button>
      </div>
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col p-6 max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => setCurrentScreen("MODE_SELECTION")} className="text-gray-800 text-sm">
            Switch Mode
          </button>
        </div>

        <div className="mb-6">
          <p className="text-gray-800 mb-2">Your lending score:</p>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {loanOffers && (loanOffers as any[]).length > 0 ? (
              ...
              <p className="text-gray-800 text-sm">No active offers</p>
            )}
          </div>
                  ...
                  <div key={tx.hash} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-700">{new Date(tx.timestamp).toLocaleTimeString()}</p>
                      </div>
                    </div>
                    <a
                      href={tx.explorerUrl}
  ...
  return <div className="relative">{userMode === "borrower" ? borrowerScreens() : null}</div>;
};
